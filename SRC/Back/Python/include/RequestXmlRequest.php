...
class RequestXmlRequest{
    public static function soapRequest($cert, $keyPEM, $token, $rfc, $fechaInicial, $fechaFinal, $TipoSolicitud = 'CFDI', $EmiRec){
        $xmlString = RequestXmlRequest::getSoapBody($cert, $keyPEM, $rfc, $fechaInicial, $fechaFinal, $TipoSolicitud, $EmiRec);
    ...
    public static function getSoapBody($cert, $keyPEM, $rfc, $fechaInicial, $fechaFinal, $TipoSolicitud, $EmiRec){
      if ($EmiRec == 'emisor') {
      ...
      if ($EmiRec == 'receptor') {
        $dataToHash = '<des:SolicitaDescarga xmlns:des="http://DescargaMasivaTerceros.sat.gob.mx"><des:solicitud RfcReceptor="'.$rfc.'" RfcSolicitante="'.$rfc.'" FechaInicial="'.$fechaInicial.'" FechaFinal="'.$fechaFinal.'" TipoSolicitud="'.$TipoSolicitud.'"></des:solicitud></des:SolicitaDescarga>';
        }
      ...
      if ($EmiRec == 'receptor') {
        $xml = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" xmlns:des="http://DescargaMasivaTerceros.sat.gob.mx" xmlns:xd="http://www.w3.org/2000/09/xmldsig#"><s:Header/><s:Body><des:SolicitaDescarga><des:solicitud RfcReceptor="'.$rfc.'" RfcSolicitante="'.$rfc.'" FechaFinal="'.$fechaFinal.'" FechaInicial="'.$fechaInicial.'" TipoSolicitud="'.$TipoSolicitud.'"><Signature xmlns="http://www.w3.org/2000/09/xmldsig#"><SignedInfo><CanonicalizationMethod Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#"></CanonicalizationMethod><SignatureMethod Algorithm="http://www.w3.org/2000/09/xmldsig#rsa-sha1"></SignatureMethod><Reference URI="#_0"><Transforms><Transform Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#"></Transform></Transforms><DigestMethod Algorithm="http://www.w3.org/2000/09/xmldsig#sha1"></DigestMethod><DigestValue>'.$digestValue.'</DigestValue></Reference></SignedInfo><SignatureValue>'.base64_encode($digs).'</SignatureValue><KeyInfo><X509Data><X509IssuerSerial><X509IssuerName>'.$datos.'</X509IssuerName><X509SerialNumber>'.$serialNumber.'</X509SerialNumber></X509IssuerSerial><X509Certificate>'.base64_encode($cert).'</X509Certificate></X509Data></KeyInfo></Signature></des:solicitud></des:SolicitaDescarga></s:Body></s:Envelope>';
        }

      // $fh = fopen("Datos.xml", 'w') or die("Se produjo un error al crear el archivo");
      // fwrite($fh, $xml) or die("No se pudo escribir en el archivo");
      // fclose($fh);
